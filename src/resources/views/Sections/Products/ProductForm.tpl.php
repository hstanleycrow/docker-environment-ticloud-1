...
if (isset($_SESSION['formData']['price'])) :
    $price = $_SESSION['formData']['price'] ?? 0;
    unset($_SESSION['formData']['price']);
endif;
...
<div class="container mt-5">
    <form method="POST" action="<?= $formAction; ?>" class="needs-validation" enctype="multipart/form-data" novalidate>
        <? if ($action == 'edit') : ?>
            ...
            <div class="mb-3 has-validation">
                <label for="active" class="form-label">Producto activo:</label>
                <div class="col-sm-4">
                    <?= $active; ?>
                </div>
        ...
        <div class="mb-3 has-validation">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control <?= (!empty($errors['name'])) ? 'is-invalid' : '.is-valid'; ?>" name="name" autocomplete="off" value="<?= htmlspecialchars($name); ?>" aria-describedby="name">
            </div>
        ...
        <div class="mb-3 has-validation">
            ...
            <div class="col-sm-4">
                <textarea name="description" id="description" cols="30" rows="5" class="form-control <?= (!empty($errors['description'])) ? 'is-invalid' : '.is-valid'; ?>"" aria-describedby=" description"><?= htmlspecialchars($description); ?></textarea>
            </div>
        ...
        <div class="mb-3 has-validation">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control <?= (!empty($errors['price'])) ? 'is-invalid' : '.is-valid'; ?>" name="price" autocomplete="off" value="<?= $price; ?>" aria-describedby="price" placeholder='0.00'>
            </div>
            <? if (!empty($errors['price'])) : ?>
                <div id="price" class="text-danger text-muted">
                    <?= $errors['price']; ?>
                </div>
            <? endif; ?>
        </div>
